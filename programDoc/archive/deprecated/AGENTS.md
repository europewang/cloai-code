...
7. 权限主干完善 ：支持按 DATASET/DATASET_OWNER/SKILL/MEMORY_PROFILE 授权
8. 契约与实现对齐 ：修正权限接口入参
9. 数据模型补齐 ： tool_call_audits 、 rag_query_audits 已落地
10. RagFlow 深度接入 ：完成检索代理接口与审计接入
11. 文件网关验证 ： upload -> indicator-verification -> download 全链路
12. 部署编排约束 ：仅启动 brain + ragflow
13. 对象清理 ：新增 ops:cleanup-s3-orphans
14. 维护任务 ：新增回填、清理、迁移脚本
15. 缺失文件治理 ：标记 missing 文件、管理接口
16. 集成测试 ：新增 run_governance_e2e.py
17. 策略版本与细分审计 ：Redis 版本号、细分审计表
18. 前端联调 ：容器启动、编排收敛、接口兼容改造
19. 模型运行策略调整 ：xinference 仅两小模型 + Ollama 大模型
20. skills 运行器契约统一 ：统一输入输出格式
21. 端到端流程联测 ：多用户测试
22. 前端适配层改造 ：新增 agent/tool 兼容实现
23. 多用户记忆改造：记忆按 memory_profile 隔离
24. Brain Service Docker 部署修复：skills 目录挂载、skill 名称匹配兼容
25. 流式输出架构：SSE 真正流式输出，修复事件重复问题
26. 前端 Markdown 表格支持：remark-gfm 插件安装与样式配置
27. 知识库权限过滤 Bug 修复（2026-05-12）：修复非授权用户可见大量未授权库的权限漏洞
### 5.3 进行中内容
