...
export function ModelPicker(t0) {
  ...
  const rawModelOptions: PickerOption[] = (customOptions !== undefined ? customOptions : t3) as PickerOption[];
  ...
  let t4;
  bb0: {
    if (initial !== null && !modelOptions.some(opt => opt.value === initialValue)) {
      let t5;
      if ($[4] !== initial) {
  ...
  if ($[35] !== effort || $[36] !== hasToggledEffort || $[37] !== onSelect || $[38] !== setAppState || $[39] !== skipSettingsWrite) {
    t14 = function handleSelect(value_0) {
      ...
      const selectedModel = resolveOptionModel(value_0, modelOptions);
      const selectedEffort = hasToggledEffort && selectedModel && modelSupportsEffort(selectedModel) ? effort : undefined;
      if (value_0 === NO_PREFERENCE) {
        onSelect(null, selectedEffort);
        return;
      }
      onSelect(customOptions !== undefined ? value_0 : selectedModel ?? value_0, selectedEffort);
    };
    $[35] = effort;
    $[36] = hasToggledEffort;
    $[37] = onSelect;
